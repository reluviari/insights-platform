...
**Seed Mongo (dev):** com `docker compose up --build` na raiz, **mongo-seed** corre antes da API — dados base em Mongo (**Keycloak não está em uso**; campos `realmId` são para eventual SSO futuro). Ver [docker/KEYCLOAK.md](../docker/KEYCLOAK.md) só quando for relevante.
